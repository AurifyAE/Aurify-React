  ...
  <div className="bg-white rounded-lg shadow-md p-4 flex justify-between items-center">
    <div>
      ...
      <div className="flex items-center">
        ...
        {additionalInfo && <span className="text-green-500 text-sm ml-2">{additionalInfo}</span>}
      </div>
    </div>
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen flex-1 overflow-x-hidden">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <MetricCard title="My Wallet" value="$0" icon={Wallet} />
        <MetricCard title="Number Of Traders" value="0" icon={Users} additionalInfo="+4" />
        <MetricCard title="Spot Orders" value="23" icon={FileText} additionalInfo="0%" />
        <MetricCard title="Txn Requests" value="34" icon={ShoppingCart} additionalInfo="0%" />
      </div>
      ...
      <div className="bg-teal-900 rounded-lg p-6 text-white mb-6">
        ...
        <div className="flex space-x-4 mb-4">
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium text-sm ${activeTab === 'All' ? 'bg-transparent text-purple-600' : 'bg-transparent border-gray-600 text-gray-300'}`}
          >
            All
          </button>
          <button
            onClick={() => setActiveTab('Pending')}
            className={`px-4 py-2 rounded-lg font-medium text-sm ${activeTab === 'Pending' ? 'bg-transparent text-purple-600' : 'bg-transparent border-gray-600 text-gray-300'}`}
          >
            Pending
          </button>
          <button
            onClick={() => setActiveTab('Completed')}
            className={`px-4 py-2 rounded-lg font-medium text-sm ${activeTab === 'Completed' ? 'bg-transparent text-purple-600' : 'bg-transparent border-gray-600 text-gray-300'}`}
          >
            Completed
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800 rounded-lg shadow-md p-4">
          <h2 className="text-white text-lg font-semibold mb-2">Active Clients</h2>
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <h2 className="text-gray-800 text-lg font-semibold mb-2">Sales Overview</h2>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={salesData}>
              ...
              <Tooltip />
              <Area type="monotone" dataKey="Gold" stroke="#8884d8" fillOpacity={1} fill="url(#colorValue1)" />
              <Area type="monotone" dataKey="Silver" stroke="#82ca9d" fillOpacity={1} fill="url(#colorValue2)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
