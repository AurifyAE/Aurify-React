...
const TVView = () => {
  ...
  const calculateUserSpotRatePrice = useCallback((commodity, type) => {
    const isGoldRelated = commodity.metal.toLowerCase().includes("gold") || commodity.metal.toLowerCase().includes("minted bar");
    const metal = isGoldRelated ? "Gold" : commodity.metal || "Unknown";
    const metalPrice = marketData[metal] && marketData[metal].bid ? parseFloat(marketData[metal].bid) : 0;

    const adjustedMetalPrice = type === "sell"
      ? metalPrice + parseFloat(getSpreadOrMarginFromDB(metal, "bid")) + (isGoldRelated ? 0.5 : 0.05)
      : metalPrice;
  ...
  const renderSpotRate = (metal) => {
    const capitalizedMetal = metal.charAt(0).toUpperCase() + metal.slice(1).toLowerCase();
    const bidSpread = parseFloat(getSpreadOrMarginFromDB(capitalizedMetal, "bid")).toFixed(2);
    const askSpread = parseFloat(getSpreadOrMarginFromDB(capitalizedMetal, "ask")).toFixed(2);
    
    const baseBidPrice = parseFloat(marketData[capitalizedMetal]?.bid) || 0;
    const bidPrice = (parseFloat(baseBidPrice) + parseFloat(bidSpread)).toFixed(2);
    
    const baseAskPrice = parseFloat(bidPrice) || 0;
    const askAdjustment = capitalizedMetal === 'Gold' ? 0.5 : 0.05;
    const askPrice = (parseFloat(baseAskPrice) + parseFloat(askSpread) + parseFloat(askAdjustment)).toFixed(2);
    return (
    <Grid item xs={6}>
      <SpotRateCard>
        <CardContent>
          ...
          <Grid container spacing={2}>
            <Grid item xs={6}>
              ...
              <Typography variant="body2" color="white">
                LOW: ${marketData[metal.charAt(0).toUpperCase() + metal.slice(1).toLowerCase()]?.low || 'N/A'}
              </Typography>
            </Grid>
            <Grid item xs={6}>
              <Typography variant="subtitle1" color="white">
              ...
              <Typography variant="body2" color="white">
                HIGH: ${marketData[metal.charAt(0).toUpperCase() + metal.slice(1).toLowerCase()]?.high || 'N/A'}
              </Typography>
            </Grid>
          </Grid>
        </CardContent>
          ...
          {isLoading ? (
            ...
            commodities.map((item, index) => (
              <TableRow key={index}>
                ...
                <TableCell>
                  <Typography>{calculateUserSpotRatePrice(item, "buy")}</Typography>
                </TableCell>
                <TableCell>
                  <Typography>{calculateUserSpotRatePrice(item, "sell")}</Typography>
                </TableCell>
              </TableRow>
            ))
          ) : (
